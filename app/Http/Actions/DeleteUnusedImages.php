...
class DeleteUnusedImages extends Controller {
    ...
    public function cleanAnuncioDirectory(){


        $files = Storage::files('public/'.config('filesystems.anunciosImageDir'));
        ...
        foreach ($files as $file) {
            ...
            $imageHasAnuncio = Anuncio::where('image', 'like', $ex[3] )->first();
            if( $imageHasAnuncio ) {
                continue;
            }else{
                // dd($ex);
                Storage::delete('public/'.config('filesystems.anunciosImageDir').'/'.$ex[3] );
